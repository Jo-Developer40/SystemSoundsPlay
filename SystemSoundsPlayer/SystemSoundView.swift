    //
    //  SystemSoundsPlayerApp.swift
    //  SystemSoundView
    //
    //  Created by Juergen on 20.09.25.
    //
...
class SystemSoundPlayer: ObservableObject {
    ...
    @Published var favoriteSounds: [Int] = [] {
        didSet {
            saveFavorites()
        }
    }
    ...
    func addFavoriteSound() {
        guard favoriteSounds.count < 5 else { return }
            ...
            ScrollView {
                    ...
                    HStack(spacing: 15) {
                        ...
                        Button("Set") {
                            if let newSoundID = Int(inputSoundID), newSoundID >= 1000, newSoundID <= 2000 {
                                ...
                                AudioServicesPlaySystemSound(1053) // Warnsound
                            }
                ...
                if let selectedSound = soundPlayer.selectedSoundID {
                    Text("Ausgewählter Sound: \(selectedSound)")
                        .font(.subheadline)
                        .padding(.top)
                    Text("Code: AudioServicesPlaySystemSound(\(selectedSound))")
                        .font(.subheadline)
                }
            }
